...
export async function DashboardSection({ month }: { month?: string }) {
  ...
  const baseDate = month
    ? new Date(`${month}-15T00:00:00`)
    : now
  const currentMonthKey = format(baseDate, 'yyyy-MM')
  const thisMonthStart = format(startOfMonth(baseDate), 'yyyy-MM-dd')
  const thisMonthEnd = format(endOfMonth(baseDate), 'yyyy-MM-dd')
  const lastMonthStart = format(startOfMonth(subMonths(baseDate, 1)), 'yyyy-MM-dd')
  const lastMonthEnd = format(endOfMonth(subMonths(baseDate, 1)), 'yyyy-MM-dd')
  ...
  const mesActual = format(baseDate, 'MMMM yyyy', { locale: dateLocale })
